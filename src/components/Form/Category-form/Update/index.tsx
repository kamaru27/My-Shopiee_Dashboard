...
const mySchema = z.object({
  categoryName: string().trim().min(1, { message: "Category name is required." }),
  categoryDescription: string().trim(),
  categoryImage: z.union([z.any().refine((file) => file?.size <= MAX_FILE_SIZE, 'Max image size is 5MB.')
    .refine(
...
const CategoryEditForm = ({categoryId,getBrand: getCategory}:Props) => {
