...
const ViewPage = async ({ getProduct, categoryId }: Props) => {
  ...
  return (
    <>
      ...
      <div className="h-[43rem] w-full rounded-xl border-b border-stroke bg-white pt-5 text-center text-xl font-semibold text-black dark:border-stroke-dark dark:bg-gray-dark dark:text-white mt-10">
        Categories
        <div className=" mx-6 grid grid-cols-4">
          {getProduct.map((item: any, index: any) => (
            <Link href={`/admin/products/view/${item._id}`} key={index}>
              <div className="mt-[10px] h-75 w-60 rounded-lg bg-gray-3 dark:bg-gray-6">
                <div className="relative left-6 top-5 size-[190px]">
                  <Image
          ...
          {getProduct.length==!1? <p>There is no products in this category</p>:""}
